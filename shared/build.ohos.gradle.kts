...
kotlin {
    ...
    cocoapods {
        summary = "Demo host module for KuiklyTable."
        homepage = "https://github.com/AriaLEntropy/KuiklyTableView"
        version = "1.0"
...
group = "com.arialentropy.kuiklytable"
version = System.getenv("kuiklyBizVersion") ?: "1.0.0"

publishing {
    repositories {
        val mavenRepoUrl = rootProject.properties["MAVEN_REPO_URL"]?.toString().orEmpty()
        if (mavenRepoUrl.isNotBlank()) {
            maven {
                url = uri(mavenRepoUrl)
                credentials {
                    username = rootProject.properties["MAVEN_USERNAME"]?.toString().orEmpty()
                    password = rootProject.properties["MAVEN_PASSWORD"]?.toString().orEmpty()
                }
            }
        }
    }
...
android {
    namespace = "com.arialentropy.kuiklytable.shared"
    compileSdk = 34
    defaultConfig {
        minSdk = 21
        targetSdk = 30
